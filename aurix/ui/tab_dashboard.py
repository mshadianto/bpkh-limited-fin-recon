...
def render_dashboard_tab(summary: Dict[str, Any], coa_recon: pd.DataFrame):
    ...
    with col_a:
        st.metric("Total Net Variance", f"SAR {summary['total_net_variance']:,.2f}")

    with col_b:
        lv_coa = int(summary['largest_variance_coa']) if summary['largest_variance_coa'] else 'N/A'
        st.metric("Largest Variance", f"COA {lv_coa}", f"SAR {summary['largest_variance_amount']:,.2f}")

    with col_c:
        st.metric("Match Rate", f"{match_rate:.1f}%", health)
